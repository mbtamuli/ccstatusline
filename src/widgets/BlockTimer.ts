...
export class BlockTimerWidget implements Widget {
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        ...
        try {
            ...
            if (blockMetrics.source === 'api' && blockMetrics.utilization !== undefined && blockMetrics.resetsAt) {
                progress = Math.min(blockMetrics.utilization / 100, 1.0);
                remainingMs = Math.max(0, blockMetrics.resetsAt.getTime() - Date.now());
                elapsedMs = sessionDurationMs - remainingMs;
            } else {
                const now = new Date();
                elapsedMs = now.getTime() - blockMetrics.startTime.getTime();
                progress = Math.min(elapsedMs / sessionDurationMs, 1.0);
                remainingMs = Math.max(0, sessionDurationMs - elapsedMs);
            }
